...
ShellRoot {
    ...
    function execControl(command, argument) {
        ...
        p.command = [Quickshell.env("HOME") + "/.local/bin/qs-control", command]
    ...
    function displayDate() {
        return Qt.formatDateTime(new Date(), "ddd dd MMM")
    }
    ...
    function monthStartOffset() {
        // JS getDay(): Sun=0. Convert to Monday=0.
        var d = monthDate().getDay()
        return (d + 6) % 7
    }
    ...
    Process {
        id: networkKindProc
        command: [Quickshell.env("HOME") + "/.local/bin/qs-control", "network-kind"]
    ...
    Process {
        id: networkLabelProc
        command: [Quickshell.env("HOME") + "/.local/bin/qs-control", "network-label"]
        stdout: StdioCollector {
    ...
    Process {
        id: bluetoothProc
        command: [Quickshell.env("HOME") + "/.local/bin/qs-control", "bluetooth-exists"]
        stdout: StdioCollector {
            onStreamFinished: root.bluetoothAvailable = text.trim() === "yes"
        }
    }

    Process {
        id: batteryExistsProc
        command: [Quickshell.env("HOME") + "/.local/bin/qs-control", "battery-exists"]
        stdout: StdioCollector {
            onStreamFinished: root.batteryAvailable = text.trim() === "yes"
        }
    }

    Process {
        id: batteryProc
        command: [Quickshell.env("HOME") + "/.local/bin/qs-control", "battery-status"]
        stdout: StdioCollector {
            onStreamFinished: root.batteryData = text.trim()
        }
    }

    Process {
        id: outputProc
        command: [Quickshell.env("HOME") + "/.local/bin/qs-control", "audio-output"]
        stdout: StdioCollector {
            onStreamFinished: {
    ...
    Process {
        ...
        command: [Quickshell.env("HOME") + "/.local/bin/qs-control", "audio-input-exists"]
        stdout: StdioCollector {
            onStreamFinished: root.microphoneAvailable = text.trim() === "yes"
        }
    }

    Process {
        id: microphoneProc
        command: [Quickshell.env("HOME") + "/.local/bin/qs-control", "audio-input"]
        stdout: StdioCollector {
            onStreamFinished: {
                var value = text.trim()
                if (value === "none")
    ...
    PanelWindow {
        ...
        Rectangle {
            ...
            // Left cluster: launcher + workspaces only.
            Row {
            ...
            // True center clock, independent from left/right cluster widths.
            Rectangle {
                id: centerClock
                ...
                implicitHeight: 32
                radius: 11
                color: centerClockMouse.containsMouse
                       ? Theme.surface2
                       : "transparent"
                ...
                MouseArea {
                    ...
                    hoverEnabled: true
                    cursorShape: Qt.PointingHandCursor
                    onClicked: root.openPage("calendar")
                }
                ...
                Timer {
                    ...
                    triggeredOnStart: true
                    onTriggered: {
                        centerTime.text = Qt.formatDateTime(new Date(), "HH:mm")
                        centerDate.text = root.displayDate()
                    }
                }
            }

            // Right cluster: tray + hardware-aware controls.
            Row {
                ...
                BarButton {
                    label: "󰂚"
                    onTriggered: root.execControl("notifications")
                }
    ...
    PanelWindow {
        id: popup
        visible: false
        implicitWidth: root.page === "calendar" ? 430 : 410
        implicitHeight: root.page === "calendar" ? 505 : 470
        color: "transparent"
        focusable: true
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    Text {
                        ...
                        text: {
                            ...
                            if (root.page === "battery") return "󰁹  Battery"
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        MiniButton {
                            label: "󰅁"
                            implicitWidth: 46
                            onTriggered: root.calendarMonthOffset--
                        }
                    ...
                    GridLayout {
                        ...
                        Repeater {
                            ...
                            Rectangle {
                                ...
                                property int dayNumber: root.cellDay(index)

                                color: root.isToday(dayNumber)
                                       ? Theme.accent
                                       : dayNumber > 0
                                         ? Theme.surface
                                         : "transparent"
                                ...
                                Text {
                                    anchors.centerIn: parent
                                    text: parent.dayNumber > 0
                                          ? parent.dayNumber
                                          : ""
                                    color: root.isToday(parent.dayNumber)
                                           ? "#11111b"
                                           : Theme.text
                                    font.pixelSize: 12
                                    font.bold: root.isToday(parent.dayNumber)
                                }
                            }
                        }
                    }
                    ...
                    MiniButton {
                        ...
                        label: "Today"
                        implicitWidth: 90
                        onTriggered: root.calendarMonthOffset = 0
                    }
                }
                ...
                ColumnLayout {
                    ...
                    InfoCard {
                        ...
                        titleText: root.networkLabel !== ""
                                   ? root.networkLabel
                                   : "Network"
                        subText: root.networkKind.indexOf("wifi") === 0
                                 ? "Wi-Fi"
                                 : "Ethernet"
                    }
                    ...
                    PageButton {
                        ...
                        subText: "Connections / VPN / DNS"
                        onTriggered: root.execControl("network")
                    }
                ...
                ColumnLayout {
                    ...
                    PageButton {
                        ...
                        onTriggered: root.execControl("bluetooth")
                    }
                ...
                ColumnLayout {
                    ...
                    Slider {
                        ...
                        onMoved: {
                            root.outputVolume = Math.round(value)
                            root.execControl("output-set", root.outputVolume)
                        }
                    }
                    ...
                    RowLayout {
                        ...
                        MiniButton {
                            Layout.fillWidth: true
                            label: "󰓃 Mixer"
                            onTriggered: root.execControl("audio")
                        }
                    }
                    ...
                    RowLayout {
                        ...
                        Text {
                            text: root.microphoneMuted
                                  ? "Muted"
                                  : root.microphoneVolume + "%"
                            color: root.microphoneMuted
                                   ? Theme.danger
                                   : Theme.subtext
                        }
                    }
                    ...
                    Slider {
                        ...
                        onMoved: {
                            root.microphoneVolume = Math.round(value)
                            root.execControl("input-set", root.microphoneVolume)
                        }
                    }
                ...
                ColumnLayout {
                    ...
                    InfoCard {
                        ...
                        titleText: {
                            var p = root.batteryData.split("|")
                            return "Battery " + (p[0] || "0") + "%"
                        }
                        subText: {
                            var p = root.batteryData.split("|")
                            return p[1] || "unknown"
                    ...
                    PageButton {
                        ...
                        onTriggered: root.execControl("power")
                    }
                ...
                PageButton {
                    ...
                    onTriggered: root.execControl("display")
                }
                ...
                PageButton {
                    ...
                    onTriggered: root.execControl("session")
                }
...
Rofi:
  ...
  - wallpaper picker uses 4 x 3 thumbnail grid
